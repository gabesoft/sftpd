...
function Sftp(options, cbconnect) {
    ...
    this.queue           = [];
    this.cmdOptions      = { ls: [] };
    this.log             = options.log || new SilentLog();
    this.tempPrefix      = this.options.host + '.sftp';
...
require("util").inherits(Sftp, EventEmitter);
...
(function() {
    ...
    this.connect = function(cbconnect) {
        ...
        this.log.debug("launching: sftp " + args.join(" "));
    ...
    this.readFile = function(filename, encoding, callback) {
        ...
        this.exec("readFile", "get " + filename + " " + tempName, function(lines) {
            var err = isError(lines);
            if (err) { return callback(err); }
            fs.readFile(tempName, encoding, function(err, data) {
                if (err) { return callback(err); }
 ...
 * @type {Boolean}
    ...
    this.writeFile = function(filename, data, encoding, callback) {
        ...
        temp.open(_self.tempPrefix, function(err, info) {
            if (err) { return callback(error); }
            var buffer = new Buffer(data, encoding);
            fs.write(info.fd, buffer, 0, buffer.length, 0, function(err) {
                if (err) { return callback(err); }
                _self.exec("writeFile", "put " + info.path + " " + filename, function(lines) {
                    callback(isError(lines));
                });
            });
        });
