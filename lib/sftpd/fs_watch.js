...
function FsWatch (opts) {
    ...
    this.ignore        = function(file) { return true; };
    this.watched       = [];
...
FsWatch.prototype.start = function() {
    ...
    if ((self.noWatch || self.watchWorks) && !self.legacy) {
        self.change(self.lastDate, function(files) {
            if (files.length > 0) {
                self.log.info('changes detected A: ' + files.length);
                //files = filter(this.ignore); // do we ignore here or in changeFunction?
        ...
        self.changedSince(self.lastDate, function(files) {
            if (files.length) {
                ...
                self.setTimeout(function() {
                    self.log.info('changes detected B: ' + files.length);
                    files.forEach(function(file) {
                        self.emit('change', file);
                    });
                    self.lastDate = +new Date();
                    // todo: do we need setTimeout here?
                    self.setTimeout(self.start, self.timeout);
                }, self.delay);
            } else {
                self.setTimeout(self.start, self.timeout);
    ...
    if (this.noWatch) {
        return function(last, cb) {
            ...
            self.dirs.forEach(function(dir) {
                var cmd = _s.sprintf(
                        'find -L "%s" -type f -mtime -%ss -print'
                      , dir
